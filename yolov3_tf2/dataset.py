...
def transform_targets_for_output(y_true, grid_size, anchor_idxs):
    ...
    idx = 0
    for i in tf.range(N):  # batch
        for j in tf.range(tf.shape(y_true)[1]):  # max bboxes
            # skip dummy bbox
            if tf.reduce_all(tf.equal(y_true[i][j][:4], 0)):
                continue
...
def parse_tfrecord(tfrecord, class_table, size):
    ...
    tf.debugging.assert_non_negative(labels)
